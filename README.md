...
- [X] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
      ...
      The store is the only place the state is held.

- [X] What is the difference between Application state and Component state? When would be a good time to use one over the other?

      Application state is global and Component state is local.  Application state is good to use when using multiple components that need to access the state.     

- [X] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

      Redux Thunk allows you to create action creators that return functions instead of actions.

- [X] What is your favorite state management system you've learned and this sprint? Please explain why!

      Redux is my favorite because it allows you pass the state and actions into multiple components asynchronously.
